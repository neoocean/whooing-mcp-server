...
claude mcp add whooing --scope user \
  -- /abs/path/to/whooing-mcp-server/bin/whooing-mcp-remote.sh
```
...
```json
{
  "mcpServers": {
    ...
    "whooing-extras": {
      "command": "/abs/path/to/whooing-mcp-server/.venv/bin/python",
      "args": ["-m", "whooing_mcp"]
    }
  }
}
...
claude mcp add whooing-extras --scope user \
  -- /abs/path/to/whooing-mcp-server/.venv/bin/python -m whooing_mcp
```
