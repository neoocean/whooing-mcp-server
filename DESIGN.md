...
| **pending queue** (CL #8) | **로컬 SQLite (영구)** | `<project>/whooing-data.sqlite` |
| **entry annotations** (note + hashtags) | **로컬 SQLite (영구)** | 같은 파일, 별도 테이블 (§6.7) |
...
| **PDF 임포트** | P1 | **✅ 완료 — `whooing_reconcile_pdf` + `whooing_pdf_format_detect` (신한/현대)** |
